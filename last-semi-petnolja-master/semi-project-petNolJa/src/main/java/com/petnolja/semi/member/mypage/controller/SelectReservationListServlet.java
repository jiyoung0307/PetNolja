...
@WebServlet("/reservation/list/select")
public class SelectReservationListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<ReservationDTO> reservationList = new MypageService().selectUserReservationList(userNo);
		
		request.setAttribute("reservationList", reservationList);
		request.getRequestDispatcher("/WEB-INF/views/member/mypage/reservationList.jsp").forward(request, response);
	}
